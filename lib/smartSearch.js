...
export async function searchContent(query, options = {}) {
  ...
  try {
    ...
    const results = documents.map(doc => {
      ...
      // For headless WordPress with Faust.js, we need to be more careful about URL generation
      let url;
      
      if (doc.data.post_name) {
        // Use the post slug for both posts and pages
        url = `/${doc.data.post_name}/`;
        console.log(`🔗 Generated URL from post_name: ${url}`);
      } else {
        ...
        console.log(`🔗 Generated URL from title: ${url}`);
      }
