...
export async function searchContent(query, options = {}) {
  ...
  try {
    ...
    const results = documents.map(doc => {
      // Generate proper URL based on post type and slug
      let url;
      if (doc.data.post_name && doc.data.post_type === 'page') {
        // For pages, use the slug directly
        url = `/${doc.data.post_name}/`;
      } else if (doc.data.post_name && doc.data.post_type === 'post') {
        // For posts, use the slug directly (assuming pretty permalinks)
        url = `/${doc.data.post_name}/`;
      } else {
